...
if __name__ == "__main__":
    ...
    ####configurations
    wandb.init(group="120620-2_5bot", project="rl-ppo-federated")
    wandb.run.name = wandb.run.id
    wandb.run.notes ="PPO max version running on half complexity in the neural net,5-bot larger, 30 simul run for 400"
    ...
    for z in range(wandb.config.epochs):
        ...
        # train the agent, take the max reward actor/critic
        for j in range(len(agents)):
