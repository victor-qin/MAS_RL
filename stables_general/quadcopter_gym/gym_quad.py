...
class GymQuad(gym.Env):
    ...
    def __init__(self, Tf = 3.0, isTrack = False):
        self.Ti = 0
        self.Ts = 0.005
        self.Tf = Tf
        self.t = self.Ti
        ...
        self.spin_err_const = 0.01
        self.vel_err_const = 0.01
    ...
    def _get_reward(self, state):
        ...
        # pos_err = self.target[0:3] - state[0:3]
        # spin_err = state[10:13]
        # vel_err = state[7:10]
        ...
        reward = np.maximum(0.0, 1 - pos_err) - self.spin_err_const * spin_err - self.vel_err_const * vel_err

        # reward = - (self.pos_err_const * pos_err)
        # reward = - (self.pos_err_const * pos_err +
        #             self.spin_err_const * spin_err + 
        #             self.vel_err_const * vel_err)
        # print(np.matmul(pos_err, np.matmul(np.eye(3), np.transpose(pos_err))))
        # reward = - (self.pos_err_const * np.matmul(pos_err, np.matmul(np.eye(3), np.transpose(pos_err))) +
        #             self.spin_err_const * np.matmul(spin_err, np.matmul(np.eye(3), np.transpose(spin_err))) + 
        #             self.vel_err_const * np.matmul(vel_err, np.matmul(np.eye(3), np.transpose(vel_err))))
