...
def main():
    ...
    ##### Config Stuff
    # eps0.15-critic
    group_temp = "032021-4_test"
    env_id = 'Pendulum-v0'

    wandb.init(group=group_temp, project="rl-ddpg-federated", mode="offline")
    wandb.run.name = wandb.run.id
    wandb.run.notes ="Testing DDPG running some sort of averaging"

    wandb.config.gamma = 0.98
    wandb.config.actor_lr = 0.001
    wandb.config.critic_lr = 0.001
    wandb.config.buffer_size = 200000
    wandb.config.learning_starts = 10000
    wandb.config.gradient_steps = -1
    wandb.config.noise_std = 0.1
    ...
    wandb.config.actor = {'layer1': 400, 'layer2' : 300}
    wandb.config.critic = {'layer1': 400, 'layer2' : 300}
    ...
    for _ in range(wandb.config.num_agents):
        ...
        # NEED TO DEFINE A NOISE
        n_actions = env.action_space.shape[-1]
        action_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma=wandb.config.noise_std * np.ones(n_actions))
        model = DDPG("MlpPolicy", env, verbose=0,
                    ...
                    # train_freq = wandb.config.n_steps,
    ...
    for z in range(wandb.config.epochs):
        ...
        if wandb.config.average == "max":
            ...
            print("epsilon", wandb.config.epsilon)
        ...
        if wandb.config.average is not None:
            # do averaging  ***NOTE the if statements
            mean_params = dict(
                    (
                        name,
